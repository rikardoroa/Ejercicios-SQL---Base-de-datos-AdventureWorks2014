...
SELECT  FirstName,LastName,HireDate,BASERATE, BirthDate FROM DIMEMPLOYEE
 WHERE BASERATE=13.45 AND DATEDIFF(YEAR,BIRTHDATE,GETDATE())<=33
 ...
 FETCH NEXT FROM BASERATE 
  INTO @Firstname,@Lastname,@Hiredate,@BASERATE	,@birthdate
   ...
   BEGIN
	 ...
	 begin
	 UPDATE DimEmployee SET BASERATE=17.77 WHERE @BASERATE=BASERATE AND @Firstname=Firstname AND @Lastname=Lastname AND @HIREDATE=HireDate AND @BIRTHDATE=BIRTHDATE
	   end
